...
var MCPClient = class {
	...
	*/
	...
	async initialize() {
		...
		for (const server of servers) {
			require_logger.logger.info(`connecting to server ${server.name || server.url || server.path || "default"}`);
		...
		try {
			...
			if (server.command && server.args) {
				...
				if (renderedServer.auth?.type === "oauth") {
					...
					require_logger.logger.debug("[MCP] Fetching OAuth token");
				...
				try {
					...
					await client.connect(transport, requestOptions);
					require_logger.logger.debug("Connected using Streamable HTTP transport");
				} catch (error) {
					require_logger.logger.debug(`Failed to connect to MCP server with Streamable HTTP transport ${serverKey}: ${error}`);
					const { SSEClientTransport } = await import("@modelcontextprotocol/sdk/client/sse.js");
					transport = new SSEClientTransport(new URL(serverUrl), hasOptions ? transportOptions : void 0);
					await client.connect(transport, requestOptions);
					require_logger.logger.debug("Connected using SSE transport");
				}
			...
			if (this.config.pingOnConnect) try {
				...
				require_logger.logger.debug(`MCP server ${serverKey} ping successful`);
			} catch (pingError) {
			...
			if (this.isDebugEnabled) require_logger.logger.error(`Failed to connect to MCP server ${serverKey}: ${errorMessage}`);
	...
	*/
	async performTokenRefresh(serverKey, oauthConfig) {
		...
		require_logger.logger.debug(`[MCP] Successfully refreshed OAuth token for server ${serverKey}`);
	}
			...
			while (true) try {
				...
				if (!retried && isAuthError && oauthConfig) {
					...
					try {
						...
						require_logger.logger.error(`[MCP] Token refresh failed for ${serverKey}: ${refreshErrorMsg}`);
					}
				}
				if (this.isDebugEnabled) require_logger.logger.error(`Error calling tool ${name}: ${errorMessage}`);
				return {
					content: "",
					error: errorMessage
				};
	...
	async cleanup() {
		for (const [serverKey, client] of this.clients.entries()) try {
			...
			if (this.isDebugEnabled) require_logger.logger.error(`Error during cleanup: ${error instanceof Error ? error.message : String(error)}`);
		}
		this.clients.clear();
	...
	*/
	async executeFunctionCallback(functionName, args, config) {
		try {
			...
			require_logger.logger.debug(`Executing function '${functionName}' with args: ${args}`);
	...
	async getOpenAiBody(prompt, context, callApiOptions) {
		...
		const maxTokensDefault = config.omitDefaults ? require_logger.getEnvString("OPENAI_MAX_TOKENS") === void 0 ? void 0 : require_logger.getEnvInt("OPENAI_MAX_TOKENS") : require_logger.getEnvInt("OPENAI_MAX_TOKENS", 1024);
				...
				for (const functionCall of functionCalls) {
					...
					if (this.mcpClient) {
						if (this.mcpClient.getAllTools().find((tool) => tool.name === functionName)) try {
							...
							require_logger.logger.debug(`MCP tool execution failed for ${functionName}: ${error}`);
					...
					if (config.functionToolCallbacks && config.functionToolCallbacks[functionName]) try {
						...
						require_logger.logger.debug(`Function callback failed for ${functionName} with error ${error}, falling back to original output`);
...
//# sourceMappingURL=chat-Cux1uti4.cjs.map