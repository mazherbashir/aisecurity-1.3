...
function getLogFiles(logDir) {
	...
	for (const entry of entries) {
		if (!entry.isFile() || !entry.name.startsWith("promptfoo-") || !entry.name.endsWith(".log")) continue;
...
function setLoggerShuttingDown(value) {
...
function setupLogDirectory() {
	...
	const logDir = getEnvString("PROMPTFOO_LOG_DIR") ? path.resolve(getEnvString("PROMPTFOO_LOG_DIR")) : path.join(configDir, "logs");
	if (!fs.existsSync(logDir)) fs.mkdirSync(logDir, { recursive: true });
	try {
		const logFiles = getLogFiles(logDir);
		if (logFiles.length >= MAX_LOG_FILES) logFiles.slice(MAX_LOG_FILES).forEach((file) => {
			try {
...
*/
function createRunLogFile(level, { date = /* @__PURE__ */ new Date() } = {}) {
	const logDir = setupLogDirectory();
	const timestamp = date.toISOString().replace(/[:.]/g, "-").replace("T", "_").split(".")[0];
	return path.join(logDir, `promptfoo-${level}-${timestamp}.log`);
}
/**
...
function initializeRunLogging() {
	try {
		...
		if (!getEnvBool("PROMPTFOO_DISABLE_DEBUG_LOG", false)) {
			state.debugLogFile = createRunLogFile("debug", { date });
			const runLogTransport = new winston.transports.File({
				filename: state.debugLogFile,
				level: "debug",
		...
		if (!getEnvBool("PROMPTFOO_DISABLE_ERROR_LOG", false)) {
			state.errorLogFile = createRunLogFile("error", { date });
			const errorLogTransport = new winston.transports.File({
				filename: state.errorLogFile,
				level: "error",
				format: winston.format.combine(winston.format.simple(), fileFormatter)
			});
...
async function closeLogger() {
	...
	try {
		const fileTransports = winstonLogger.transports.filter((transport) => transport instanceof winston.transports.File);
		...
		for (const transport of fileTransports) {
			...
			if (handler) transport.off("error", handler);
...
//#endregion
export { isNonInteractive as A, getEnvBool as C, getEvalTimeoutMs as D, getEnvString as E, getMaxEvalTimeMs as O, setConfigDirectoryPath as S, getEnvInt as T, isValidJson as _, isDebugEnabled as a, summarizeEvaluateResultForLogging as b, setLogCallback as c, normalizeFieldName as d, sanitizeObject as f, getAjv as g, extractJsonObjects as h, initializeRunLogging as i, state as j, isCI as k, setLogLevel as l, extractFirstJsonObject as m, getLogLevel as n, logRequestResponse as o, sanitizeUrl as p, globalLogCallback as r, logger as s, closeLogger as t, REDACTED as u, orderKeys as v, getEnvFloat as w, getConfigDirectoryPath as x, safeJsonStringify as y };

//# sourceMappingURL=logger-D6YuF-jw.js.map