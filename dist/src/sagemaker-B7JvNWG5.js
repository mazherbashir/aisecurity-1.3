	...
	async applyTransformation(prompt, context) {
		...
		try {
			...
			if (typeof transformFn === "string" && !transformFn.startsWith("file://")) try {
				...
				const { TransformInputType } = await import("./transform-22ugZQuZ.js").then((n) => n.r);
...
var SageMakerCompletionProvider = class extends SageMakerGenericProvider {
	...
	*/
	async callApi(prompt, context, _options) {
		const { isCacheEnabled, getCache } = await import("./cache-CTFAMBrM.js").then((n) => n.t);
		...
		if (isCacheEnabled() && !bustCache) {
			...
			const cachedResult = await (getCache ? getCache() : await import("./cache-CTFAMBrM.js").then((n) => n.t).then((m) => m.getCache())).get(cacheKey);
			if (cachedResult) {
		...
		try {
			...
			if (isCacheEnabled() && !bustCache && result.output && !result.error) {
				const cacheKey = this.getCacheKey(transformedPrompt);
				const cache = getCache ? getCache() : await import("./cache-CTFAMBrM.js").then((n) => n.t).then((m) => m.getCache());
	...
	*/
	async callEmbeddingApi(text, context) {
		...
		if (isCacheEnabled() && !bustCache) {
			...
			const cachedResult = await (await getCache ? await getCache() : await import("./cache-CTFAMBrM.js").then((n) => n.t).then((m) => m.getCache())).get(cacheKey);
			if (cachedResult) {
	...
	*/
	async cacheEmbeddingResult(result, text, context, isTransformed = false, originalText) {
		...
		if (isCacheEnabled() && !bustCache && result.embedding && !result.error) {
			...
			const cache = await getCache ? await getCache() : await import("./cache-CTFAMBrM.js").then((n) => n.t).then((m) => m.getCache());
...
//# sourceMappingURL=sagemaker-B7JvNWG5.js.map