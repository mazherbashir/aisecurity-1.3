#!/usr/bin/env node
import { E as getEnvString, T as getEnvInt, s as logger, w as getEnvFloat } from "./logger-D6YuF-jw.js";
	...
	async applyTransformation(prompt, context) {
		...
		try {
			...
			if (typeof transformFn === "string" && !transformFn.startsWith("file://")) try {
				...
				const { TransformInputType } = await import("./transform-VMXv1Odp.js");
...
var SageMakerCompletionProvider = class extends SageMakerGenericProvider {
	...
	*/
	async callApi(prompt, context, _options) {
		const { isCacheEnabled, getCache } = await import("./cache-BKxoJXjP.js");
		...
		if (isCacheEnabled() && !bustCache) {
			...
			const cachedResult = await (getCache ? getCache() : await import("./cache-BKxoJXjP.js").then((m) => m.getCache())).get(cacheKey);
			if (cachedResult) {
		...
		try {
			...
			if (isCacheEnabled() && !bustCache && result.output && !result.error) {
				const cacheKey = this.getCacheKey(transformedPrompt);
				const cache = getCache ? getCache() : await import("./cache-BKxoJXjP.js").then((m) => m.getCache());
	...
	*/
	async callEmbeddingApi(text, context) {
		...
		if (isCacheEnabled() && !bustCache) {
			...
			const cachedResult = await (await getCache ? await getCache() : await import("./cache-BKxoJXjP.js").then((m) => m.getCache())).get(cacheKey);
			if (cachedResult) {
	...
	*/
	async cacheEmbeddingResult(result, text, context, isTransformed = false, originalText) {
		...
		if (isCacheEnabled() && !bustCache && result.embedding && !result.error) {
			...
			const cache = await getCache ? await getCache() : await import("./cache-BKxoJXjP.js").then((m) => m.getCache());
...
//# sourceMappingURL=sagemaker-DWuUm4ea.js.map