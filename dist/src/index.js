...
import { m as validateFunctionCall } from "./transform-wN5Yv5Wh.js";
import { l as validateFunctionCall$1 } from "./util-BXGhj1dO.js";
import { c as setUserEmail, i as getUserEmail, o as isLoggedIntoCloud, r as getAuthor, s as promptForEmailUnverified, t as checkEmailStatusAndMaybeExit } from "./accounts-B3C0JTGp.js";
import { i as getRemoteGenerationUrl, l as shouldGenerateRemote, o as getRemoteHealthUrl, r as promptYesNo, s as neverGenerateRemote } from "./server-B4bJX8L5.js";
import { t as getBlobByHash } from "./blobs-eGOIe08Q.js";
import { a as evalsTable, c as evalsToTagsTable, d as tagsTable, i as evalResultsTable, l as promptsTable, m as getDbSignalPath, o as evalsToDatasetsTable, p as getDb, r as datasetsTable, s as evalsToPromptsTable } from "./tables-DVuwk5Db.js";
import { n as isBlobStorageEnabled, t as extractAndStoreBinaryData } from "./extractor-KQ00tPvr.js";
import { t as telemetry } from "./telemetry-Bo6PhLgD.js";
...
import { n as runRuby } from "./rubyUtils-7e4H3vNI.js";
import { t as EvalResult } from "./evalResult-D-RiEfqw.js";
...
async function startOtlpReceiverIfNeeded(testSuite) {
	...
	if (testSuite.tracing?.enabled && testSuite.tracing?.otlp?.http?.enabled && !otlpReceiverStarted) {
		...
		try {
			...
			const { startOTLPReceiver } = await import("./otlpReceiver-49ntbNiT.js");
...
async function stopOtlpReceiverIfNeeded() {
	if (otlpReceiverStarted) try {
		...
		const { stopOTLPReceiver } = await import("./otlpReceiver-49ntbNiT.js");
...
async function generateTraceContextIfNeeded(test, evaluateOptions, testIdx, promptIdx, testSuite) {
	...
	const { getTraceStore } = await import("./store-CB3P6zdV.js").then((n) => n.n);
...
async function runAssertion({ prompt, provider, assertion, test, vars, latencyMs, providerResponse, traceId, traceData }) {
	...
	if (typeof renderedValue === "string") if (renderedValue.startsWith("file://")) {
		...
		if (isJavascriptFile(filePath)) {
			...
			const { runRuby } = await import("./rubyUtils-7e4H3vNI.js").then((n) => n.t);
	...
	*/
	...
	async _runEvaluation() {
		...
		for (let index = 0; index < tests.length; index++) {
			...
			if (!testCase.provider && typeof testSuite.defaultTest === "object" && testSuite.defaultTest?.provider) {
				...
				if (isApiProvider(defaultProvider)) testCase.provider = defaultProvider;
					...
					const { loadApiProvider } = await import("./providers-CBKmxpDQ.js");
		...
		if (state.resume && this.evalRecord.persisted) try {
			const { default: EvalResult } = await import("./evalResult-D-RiEfqw.js").then((n) => n.n);
...
async function parseXlsxFile(filePath) {
	try {
		...
		if (!fs$1.existsSync(actualFilePath)) throw new Error(`File not found: ${actualFilePath}`);
		let readXlsxFile;
		try {
...
function resolvePluginConfig(config) {
	...
	for (const key in config) {
		...
		if (typeof value === "string" && value.startsWith("file://")) {
			...
			if (filePath.endsWith(".yaml")) config[key] = yaml.load(fs$1.readFileSync(filePath, "utf8"));
			else if (filePath.endsWith(".json")) config[key] = JSON.parse(fs$1.readFileSync(filePath, "utf8"));
			else config[key] = fs$1.readFileSync(filePath, "utf8");
		}
	}
	return config;
}
...
async function doRedteamRun(options) {
	...
	if (options.liveRedteamConfig) {
		...
		fs$1.mkdirSync(path$2.dirname(tmpFile), { recursive: true });
