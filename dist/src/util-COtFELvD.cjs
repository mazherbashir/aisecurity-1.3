...
function calculateOpenAICost(modelName, config, promptTokens, completionTokens, audioPromptTokens, audioCompletionTokens) {
	if (!audioPromptTokens && !audioCompletionTokens) return require_fetch.calculateCost(modelName, config, promptTokens, completionTokens, [
		...OPENAI_CHAT_MODELS,
...
function failApiCall(err) {
	if (err instanceof openai.default.APIError) {
...
function validateFunctionCall(output, functions, vars) {
	...
	const interpolatedFunctions = require_util.maybeLoadFromExternalFileWithVars(functions, vars);
...
//# sourceMappingURL=util-COtFELvD.cjs.map